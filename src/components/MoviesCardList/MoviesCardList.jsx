...
function MoviesCardList({
  ...
  function getImageLink(movie) {
    return movie.movieId ? movie.image : MOVIE_URL + movie.image.url;
  }
  ...
  return (
    <section className="movieCardList">
      <ul className="movieCardList__items">
        {searchError ? (
          <p className="movieCardList__error">{errorMessages.searchError}</p>
        ) : movies?.length === 0 ? (
          <p className="movieCardList__error">
            {isMoviesSearched || isSavedMoviesSearched
